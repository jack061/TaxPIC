...
if __name__ == '__main__':
    ...
    train_dataset = torchvision.datasets.ImageFolder(training_data_folder, transform=tranforms_train)
    train_data = DataLoader(train_dataset, batch_size=8, shuffle=True)

    test_dataset = torchvision.datasets.ImageFolder(test_data_folder, transform=tranforms_test)
    ...
    torch.save(model,
               "D:\Code\TaxPIC\model\\" + str(100 * correct / total).replace('tensor', '') + ".pkl")  # 保存整个神经网络

    # model = torch.load("D:\Code\TaxPIC\model\\" + str(100 * correct / total).replace('tensor', '') + ".pkl")
    # print(model)
