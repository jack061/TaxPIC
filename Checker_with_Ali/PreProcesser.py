...
class ImgPreProcesser:
    ...
    def  img2base64(self, img):
        cv2.imwrite('D:\\temp.png', img)
        with open('D:\\temp.png', 'rb') as f:
            base64code = base64.b64encode(f.read()).decode('utf-8')
        return base64code


    def img_recognizer(self, imageBase64):
    ...
    def preprocess(self, flag, imgPath):
        img = self.thresholder(flag, imgPath)
        img = self.denoise(img)
        return img


if __name__ == '__main__':
    imgPath = 'D:\\AliTest\\wrong\\red_m.png'
    flag = 'red'
    i = ImgPreProcesser()
    cv2.imshow('', i.thresholder(flag, imgPath))
    cv2.waitKey(0)